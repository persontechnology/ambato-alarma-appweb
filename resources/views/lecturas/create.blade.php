...
@section('breadcrumbs')
{{ Breadcrumbs::render('lecturas.create') }}
@endsection



@section('content')
<form action="{{ route('lecturas.store') }}" method="POST" id="formUser" autocomplete="off" >
    @csrf
    <div class="card">
            ...
            <div class="col-md-6 mb-1">
                <div class="form-floating form-control-feedback form-control-feedback-start">
                    ...
                    <select class="form-select @error('visto') is-invalid @enderror" name="estado" required>
                        <option value=""></option>
                        <option value="0" {{ old('visto')===0?'selected':'' }}>NO</option>
                        <option value="1" {{ old('visto')===1?'selected':'' }}>SI</option>
                    </select>
                ...
                <div class="form-floating form-control-feedback form-control-feedback-start">
                    ...
                    <select class="form-select @error('dispositivo_id') is-invalid @enderror" name="dispositivo_id" id="dispositivo_id" required>
                        ...
                        @foreach ($dispositivos as $dispositivo)
                            <option value="{{ $dispositivo->id }}"  {{ old('dispositivo_id')==$dispositivo->id?'selected':'' }} >{{ $dispositivo->nombre }}</option>
                        @endforeach
                    </select>
